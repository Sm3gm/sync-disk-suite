...
**Combined vanilla disks can still turn up as loot even with `RemoveVanillaParents` enabled.** The setting reliably removes them from shop stock, but the game populates container loot from somewhere other than the list the mod edits. You will occasionally find a combined parent in an apartment. It works normally if you install it. This is a known limitation rather than a bug report, it is cosmetic, and it is still being investigated.
...
**The Black Market Trader's stock is currently unreachable.** The game does not display the spoken password needed to get in, showing only the thief tag with no letters. This is a base game issue rather than a mod one. The trader's rotation is generated correctly and will work the moment the password shows.
...
Bug reports and suggestions: [github.com/sm3gm/sync-disk-suite](https://github.com/sm3gm/sync-disk-suite)

If you enjoy it, either of these goes to the same person:

- [ko-fi.com/sm3gm](https://ko-fi.com/sm3gm)
- [buymeacoffee.com/sm3gm](https://buymeacoffee.com/sm3gm)
